...
class NeuralPoints_changes(nn.Module):
    ...
    def query_feature(  
        ...
        if query_geo_feature:
            ...
            if query_locally:
                ...
                print("geo_features shape:", geo_features.shape)
                print("valid_indices value:", valid_indices[0], valid_indices[1])
                print("idx_clamped shape:", idx_clamped.shape)
                print("self.local_geo_features shape:", self.local_geo_features.shape)
                ...
                print(
                    "geo_features_flat shape:", geo_features_flat.shape
                )

                print(valid_indices[0].shape)
                print(valid_indices[1].shape)

                print(valid_indices[0] * nn_k + valid_indices[1])
...
class PositionalEncoder_changes(nn.Module):
...
# https://github.com/PRBonn/LocNDF
class GaussianFourierFeatures_changes(nn.Module):
    def __init__(self, config: Config) -> None:
        super().__init__()
